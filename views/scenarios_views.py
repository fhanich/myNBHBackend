        ...
        if rental_properties[0]['clusterCity'] == job_properties[0]['clusterCity']: # evaluate the scenario
            ...
            if not('error' in nearbyRestaurants):
                ...
                if isinstance(securityScores['result'],(int, float)) & (securityScores['result']>0):
                    ...
                    total+=1
                if isinstance(affordabilityScores['result'],(int, float)) & (affordabilityScores['result']>0):
                    overallScore += (affordabilityScores['result'])
                    total+=1
                if isinstance(recognition['result'],(int, float)) & (recognition['result']>0):
                    overallScore += (recognition['result'])
                    total+=1
                if isinstance(neighborhoodsScores['score'],(int, float)) & (neighborhoodsScores['score']>0):
                    overallScore += (neighborhoodsScores['score'])
                    total+=1
                if isinstance(nearbyRestaurants['result'],(int, float)) & (nearbyRestaurants['result']>0):
                    overallScore += (nearbyRestaurants['result'])
                    total+=1

                if isinstance(commuteScore,(int, float)) & (commuteScore>0):
